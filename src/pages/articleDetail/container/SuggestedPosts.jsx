      ...
      <div className='grid gap-y-5 mt-5 md:grid-cols-2 md:gap-x-5 lg:grid-cols-1'>
        {posts.map(item => (
            <div key={item._id} className='flex space-x-3 flex-nowrap items-center'>
                <img src={item.image} alt={item.title} className='aspect-square object-cover rounded-lg w-1/5' />
                <div className='text-sm font-roboto text-dark-hard font-medium'>
                    <h3 className='text-sm font-roboto text-dark-hard font-medium md:text-base lg:text-lg'>{item.title}</h3>
                    <span className='text-xs opacity-60'>{new Date(item.createdAt).toLocaleDateString("en-US", {day: 'numeric', month: 'short', year: 'numeric'})}</span>
                </div>
            </div>
        ))}
      </div>
